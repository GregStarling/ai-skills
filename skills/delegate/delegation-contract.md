...
- **Route:** exact pack candidate identity, model, effort and material serving controls.
...
The coordinator handles dependencies and integration while workers execute. Do not duplicate their assignments. Inspect completed artifacts, not only summaries. A repair packet includes the concrete failure, affected artifact, expected correction and relevant acceptance check. Simple defects go back to the same worker; repeated capability failure advances to another eligible pack entry. Architectural ambiguity goes to frontier planning, then back to an appropriate worker.
