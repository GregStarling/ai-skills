...
Use one worker by default. Swarm only when all of these hold: at least two necessary independent workstreams, settled interfaces, disjoint ownership or isolated workspaces, less expected total model usage than sequential execution, and a coordinator who integrates the result and obtains required fresh frontier review. Time saved alone is insufficient.
...
Finish prerequisite decisions before starting dependent work. Give each writer a concrete scope and acceptance check. Use new waves as capacity frees up; do not start idle workers, recursively delegate orchestration or create user-visible tasks as an implementation detail. Integrate the outputs and apply the shared review rule: economical checks/audits for research, fresh frontier review for implemented behavior. Parallel completion alone does not establish integration correctness.
