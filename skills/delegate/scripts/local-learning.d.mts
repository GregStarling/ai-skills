...
export type OrdinaryDispatchInput = {work_type?:WorkType;fallback_route?:'coordinator'|'frontier'|'worker';routing_reason?:string;host:'codex'|'claude';assignment:string;risk:'low'|'medium'|'high'|'critical';task_id:string;phase?:'execute'|'complete';bounded?:boolean;decision_bounded?:boolean;substantial?:boolean;coordinator:OrdinaryCoordinator;frontier?:OrdinaryCoordinator;cheap_reviewer?:OrdinaryCoordinator;implemented_behavior?:boolean;origin_work_type?:WorkType|'unknown';decision_evidence?:PolicyEvidence;hard_bug_handoff?:HardBugHandoff;user_model_override?:UserModelOverride;signals?:OrdinarySignals;delegation_forbidden?:boolean;workers?:Record<string,OrdinaryCoordinator>;artifact_digest?:string;review?:{verdict:'PASS'|'REPAIR'|'BLOCKED';fresh_context:boolean;model:string;effort:string|null;artifact_digest:string};legacy?:boolean};
...
export type ReviewCheckInput={task_id:string;assignment:string;work_type?:WorkType;risk?:'low'|'medium'|'high'|'critical'|'unknown'|undefined;origin_work_type?:WorkType|'unknown'|undefined;implemented_behavior?:boolean;independent_review?:boolean};
export function checkWork(input:ReviewCheckInput):{status:'checked';task_id:string;work_type:string;risk:'low'|'medium'|'high'|'critical';origin_work_type:WorkType|'unknown'|null;audit:{selected:boolean;percent:10};review_requirement:ReviewRequirement};
...
export type OrdinaryObservationV3=Omit<OrdinaryObservationV2,'schema_version'|'observation_version'> & {schema_version?:'delegate_observation.v3';observation_version?:3;work_type:WorkType;implemented_behavior?:boolean;cheap_reviewer?:OrdinaryCoordinator;artifact_files?:string[];check_evidence?:PolicyEvidence[];execution_evidence?:PolicyEvidence[];origin_work_type?:WorkType|'unknown';decision_evidence?:PolicyEvidence;hard_bug_handoff?:HardBugHandoff;user_model_override?:UserModelOverride};
