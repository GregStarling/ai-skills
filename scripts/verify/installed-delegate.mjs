...
import {ordinaryWorkflow,validateWorkflow,ordinaryInstructions} from './ordinary-comparison.mjs';
...
export async function prepareInstalledTrial(host,name,{projectRoot=null,fixtureDirectory=null,temporaryDirectory=tmpdir(),mode='auto',harvested=null,skillSource=join(root,'skills/delegate'),workflow='evidence'}={}){
 ...
 const modeInstructions=workflow===ordinaryWorkflow?ordinaryInstructions(mode):trialModeInstructions(mode);
 ...
 return {host,directory,fixtureRoot,skillPath,task,mode,workflow,harvested:harvestedPrepared,starting_artifact_digest:startingArtifactDigest??`sha256:${hash(JSON.stringify(fixtureDigests))}`,...helperInstructions(await readFile(join(skillPath,'scripts/local-learning.mjs'),'utf8')),instructionDigests,fixtureDigests,skillDigests,skill_folder_sha256:hash(JSON.stringify(skillDigests)),skill_folder_digest:await folderDigest(skillPath)};
}
...
export async function installedTrial(host,name,{exercise=null,projectRoot=null,fixtureDirectory=null,outputDirectory=null,maxModelCalls=null,mode='auto',runId=randomUUID(),timeoutMs=null,harvested=null,skillSource=join(root,'skills/delegate'),dryRun=false,preparedTrial=null,environment=null,traceBoundInspection=false,workflow='evidence'}={}){
 validateWorkflow(workflow,host);
 const ordinary=workflow===ordinaryWorkflow;
 if(ordinary&&(exercise!==null||!harvested))throw Error('ORDINARY_COMPARISON_REQUIRES_HARVESTED_FIXTURE_WITHOUT_EXERCISE');
 ...
 const prepared=preparedTrial??await prepareInstalledTrial(host,name,{projectRoot,fixtureDirectory,mode,harvested,skillSource,workflow});
 if(prepared.host!==host||prepared.mode!==mode||prepared.task.name!==name||(prepared.workflow??'evidence')!==workflow)throw Error('PREPARED_TRIAL_MISMATCH');
 ...
 if(!ordinary)for(const [file,value] of [['start-input.json',startInput],['reminder-input.json',reminderInput],['followup-input.json',{cwd:directory,host,run_id:runId}]])await writeFile(join(evaluationDirectory,file),JSON.stringify(value,null,2)+'\n');
 ...
 const scopeException=prepared.harvested&&!ordinary?{route_class:startInput.task_class,host,reason:'Evaluation-only applicability to a harvested TypeScript fixture; no route qualification.'}:null;
