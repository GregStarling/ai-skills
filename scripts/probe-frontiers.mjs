...
const probes=await Promise.all(targets.targets.map(async target=>{
  ...
  const reason=completed?'Native CLI completed; diagnostic output needs maintainer review before routing refresh.':incompatible?errorText:result.timed_out?'Native CLI probe timed out; model availability is undetermined.':`Native CLI did not complete; availability is undetermined. ${[versionError,errorText,stderr].filter(Boolean).join('\n').slice(0,1000)}`;
