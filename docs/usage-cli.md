...
`receipt-ingest` archives an exact portable receipt with independent task/time/origin context. `receipt-assess` validates native attempt, artifact and required review evidence, then returns `evaluationLedgers` references accepted by `refresh`. Pending captures never qualify routes; accepted evidence is revalidated against the current policy. See [production receipt ingestion](production-receipts.md) for request formats and pairing limits.
