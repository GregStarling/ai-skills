...
`skills/delegate/`, 14 files. Direct-default revision added 2026-09-11: direct execution is the default; ordinary delegation is limited to large multi-package investigations, independent parallel workstreams (both untested) or explicit requests, informed by two ordinary-workflow cost-proxy wins on bounded TypeScript tasks; older comparisons measured elapsed time, not comparable usage. Ordinary `dispatch` still uses actual host slots and bounded assignments independent of repository size/language; evidence-required `route` remains strict and unchanged. One `observe` call records ordinary delegated outcomes without receipt ceremony. Earlier reliability repairs retain explicit worker-completion waits. **All three targeted reliability cases passed** on a preceding guidance iteration, not this revision. See the [follow-up results](delegate-reliability-followup-results-2026-09-11.md) and [checklist](delegate-reliability-followup-plan.md).

| Digest | Value |
| --- | --- |
| Helper `folderDigest` (skill_folder_digest) | `sha256:afe7ddc5ef0f66779afee6f013ed16e162a5df7abb46f213149ecceb59a9785e` |
| `guidance_digest` (folder without `routing-pack.json`) | `sha256:d75b6e2de73fbe78ff5e8c8a341581711f1cf72a2cfec4caff33f954500849d8` |

Direct-default follow-up (2026-09-11): the entrypoint description no longer promises savings. Step 1 makes direct execution the default and limits ordinary delegation to large multi-package investigations, independent parallel workstreams (both labeled untested) or explicit requests, replacing the investigate-first instruction. Context and worker-contract guidance, README, AGENTS.md and CLAUDE.md match. This is guidance only: no helper, pack or evidence change, no model trials and no savings claim; the new guidance is not live-tested. Separately, `runProcess` no longer reports failed cleanup when macOS returns EPERM for a killed process group whose members are still exiting; it re-signals for up to 2 s until the group is gone. With CPUs saturated, the timeout test previously failed in 4 of 8 runs; after the fix it failed in 0 of 8, and a 12-run diagnostic completed cleanup every time (7 runs hit EPERM and settled). Checks: 786 tests across 45 files, typecheck, build, portable packaging/reachability and built-CLI pack validation passed. Both installed copies matched that revision (`sha256:16281d9472208993dae7c900b78ad64290d2be278a9abbd771a84269ee056f1f`); their prior versions were backed up outside skill discovery. The subsequent evidence-wording correction is now installed; see the local review-fixes note above.
...
Follow-up release checks: 784 tests across 45 files, typecheck, build, portable packaging/reachability and built-CLI pack validation passed. The installed consumer folder matched the source at that revision; its prior version was backed up outside skill discovery. The generic Python skill validator lacked PyYAML; repository YAML/frontmatter validation passed without a new dependency.
