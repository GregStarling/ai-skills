...
One execution phase covers full v1, with ordered internal checkpoints. See implementation-handoff.md.
...
Load synthetic evidence and registry records with immutable IDs and content digests; resolve all references and reject missing or tampered evidence. Preserve source, benchmark/suite/dataset/methodology and grader versions, measured/retrieved dates, model/effort/tool/serving configuration and observation counts/costs. Enforce evidence admissibility, fallback restrictions, freshness and matching role/task-class/risk/constraints/suite/treatment. Vendor self-report alone cannot qualify. Separate benchmark versions and never overwrite history. The foundation checkpoint reads fixtures only; no ingestion service is needed.

## REQ-004: Auditable risk and review requirements

Derive categorical pre-dispatch and post-change risk from declared observable facts and stable policy rule IDs; effective risk is their maximum. Enforce role and constraint eligibility including availability, tool/modalities/context/provider/local/privacy and declared cost/latency ceilings. Critical review requires a qualified independently defined family and fresh context, otherwise ESCALATION_REQUIRED. Review evidence names the reviewed artifact digest and admissible context package. An altered final artifact invalidates review. The foundation checkpoint validates supplied synthetic facts and attestations; do not claim static validation proves a real runtime context boundary.
...
Separate initial selection from replacement. Initially select the cheapest qualified candidate with a declared deterministic tie break. For replacement, apply explicit superiority requirements and allowed cost increase to a dearer challenger, or non-inferiority plus required savings to a cheaper challenger. Use paired comparable evidence with declared margins and confidence-bound provenance; statistical inference engines are implemented later in this same build under REQ-014, so The foundation checkpoint uses explicitly synthetic paired comparison observations and validates internal consistency, never inventing a confidence float. Retain an eligible incumbent when evidence is insufficient, but never preserve an unavailable, disqualified or hard-expired incumbent. Define equal-cost and no-qualified-candidate outcomes.
...
### AC-013: Insufficient or absent incumbent
...
### AC-017: No synthetic production
...
### AC-020: Claude compiler preserves validated identity
...
### AC-021: Claude unsupported setting and drift fail closed
...
### AC-022: Codex adapter is independent and deterministic
...
### AC-023: Codex unsupported version and tampering
...
### AC-024: Traceable real harvested fixtures
...
### AC-025: Replay isolation and treatment comparability
...
### AC-026: Ledger immutable and recoverable
...
### AC-027: Accepted-task economics includes failures
...
### AC-028: Paired statistical reference vectors
...
### AC-029: Reject untrusted statistical proof
...
### AC-030: Promotion margins and economic boundaries
...
### AC-031: Production qualification remains evidence-bound
...
### AC-032: Invalid incumbent cannot be retained
...
### AC-033: Official discovery changes roster only
...
### AC-034: Benchmark methodology history preserved

Requirements: REQ-013, REQ-016
...
### AC-035: Bounded worker and fresh review contract
...
### AC-036: Scope and risk escalation prevents acceptance
...
### AC-037: Final artifact invalidates previous review
...
### AC-038: Dispatch failure remains auditable
...
### AC-039: Shadow work never substitutes for incumbent
...
### AC-040: Unsafe shadow work refused
...
### AC-041: Refresh hold is a truthful result
...
### AC-042: Refresh verifies before local activation
...
### AC-043: Offline end-to-end regression evidence
...
### AC-044: Live CLI subscription smoke and honest limits

Requirements: REQ-010, REQ-011, REQ-012, REQ-017, REQ-020
...
- Plan one full-v1 execution phase containing all REQ-001 through REQ-020 and AC-001 through AC-044. Preserve the mandated architectural order with task dependencies and an enforced foundation checkpoint before adapter work, rather than dividing the package into separate phase plans. This resolves the observed phase-plan versus full-package grill scope conflict; Foreman still owns task decomposition and worker execution.
...
- Read docs/implementation-handoff.md before authoring the task graph. It consolidates concrete supervisor review fixes: complete CLI wiring ownership, integration repair scope, silent npm JSON capture, nonempty executed test evidence, actual local subprocess dispatch, and honest final verification.
- Portable library source belongs under skills/delegate/SKILL.md and skills/refresh-models/SKILL.md, with shared name/description frontmatter supported by Claude and Codex. Do not auto-enable the skills in this development checkout by writing .claude/skills, .codex/skills or .agents/skills. Generated runtime workspaces may receive explicitly compiled configuration as part of bounded tests.
- README.md is required and must include the verified benchmark-methodology anecdote, tested local usage, both skill entry points, and the distinct outcomes of configuration conformance, live CLI execution, and empirical production qualification.
- Final proof must reject empty or fabricated passing reports, unknown status values, missing invocation/grader evidence, missing or tampered artifacts, skipped checks counted as passed, and failed offline scenarios. Coverage must reference actual executed checks and hashed artifacts; recognized blocked states need observed evidence. Available authenticated providers must receive bounded objectively graded qualification-evaluation smoke calls even when production qualification is insufficient.
