...
import { parseBinding, parseCandidate, type Candidate, type RuntimeReport } from "../schema/index.js";
import { validateBinding, type SelectionInput } from "../governance/index.js";
import { inventory, readNativeConfiguration, renderClaude, renderCodex, runtimeVersions, safeOutputDirectory, verifyRendered, type NativeProvider, type RenderedAdapter } from "../adapters/index.js";
...
export function nativeEnvironment(provider: NativeProvider, candidate: Candidate, inherited: NodeJS.ProcessEnv = process.env): { env: NodeJS.ProcessEnv; overrideNames: string[] } {
  ...
  if (provider === "anthropic") { env["CLAUDE_CODE_SUBAGENT_MODEL"] = candidate.model_id; env["CLAUDE_CODE_EFFORT_LEVEL"] = candidate.effort; names.push("CLAUDE_CODE_SUBAGENT_MODEL", "CLAUDE_CODE_EFFORT_LEVEL"); }
...
export async function runNative(input: NativeInput): Promise<NativeExecution> {
  ...
  if (probe.exit_code !== 0 || version !== runtimeVersions[input.provider]) throw new Error("RUNTIME_VERSION_MISMATCH");
  ...
  if (input.mode === "production") {
    ...
    if (candidate.material_serving_settings.includes("fallback")) throw new Error("PRODUCTION_FALLBACK_CONTROL_UNVERIFIED");
  }
  ...
  const failed = (input.mode === "production" && telemetry.identity_status !== "matched") || processResult.spawn_error !== null || processResult.output_limited || processResult.exit_code !== 0 || telemetry.provider_error || telemetry.malformed_events > 0 || packageChanged || processResult.cleanup !== "complete";
  const report: RuntimeReport = { schema_version: "runtime_report.v1", report_id: `runtime_${randomUUID().replaceAll("-", "")}`, provider: input.provider, candidate_id: candidate.candidate_id, started_at: processResult.started_at, completed_at: processResult.completed_at, command: { executable, args, cwd: resolve(input.cwd) }, status: processResult.timed_out ? "timed_out" : failed ? "failed" : "completed", exit_code: processResult.exit_code, signal: processResult.signal, timeout_ms: input.timeoutMs, stdout_digest: processResult.stdout_digest, stderr_digest: processResult.stderr_digest, observed_identity: telemetry.identity_status === "matched" ? { model_id: candidate.model_id, source: "provider_receipt" } : { source: "unknown" }, usage: { ...(telemetry.input_tokens === null ? {} : { input_tokens: telemetry.input_tokens }), ...(telemetry.output_tokens === null ? {} : { output_tokens: telemetry.output_tokens }), cost_usd: telemetry.cost_usd } };
  const blockers = [ ...(telemetry.identity_status !== "matched" ? ["SERVED_MODEL_UNVERIFIED"] : []), "PROVIDER_FALLBACK_CONTROL_UNVERIFIED", ...(input.contextPackage && input.provider === "openai" ? ["GLOBAL_SKILL_CONTEXT_UNVERIFIED"] : []), ...(telemetry.cost_usd === null ? ["COST_UNKNOWN"] : ["COST_IS_PROVIDER_ESTIMATE"]), ...(packageChanged ? ["REVIEW_PACKAGE_DRIFT"] : []) ];
  const result: NativeExecution = { ...telemetry, schema_version: "native_execution.v1", mode: input.mode, report, report_digest: digest(report), stdout_path: processResult.stdout_path, stderr_path: processResult.stderr_path, runtime_version: version, cleanup: processResult.cleanup, invocation: { argv: args, environment_override_names: overrideNames, bridge: "explicit-cli-bridge", sandbox: input.sandbox }, context: { fresh_process: true, prompt_digest: digest(input.prompt), package_digest: packageDigest, artifact_digest: input.contextPackage?.artifactDigest ?? null, instruction_scope: packageDigest === null || packageChanged || input.provider === "openai" ? "runtime-managed" : "isolated-package" }, qualification_blockers: blockers };
