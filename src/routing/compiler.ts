...
export function compileRoutingPack(input:CompileRoutingPackInput):RoutingPack{
  ...
  for(const raw of input.strata){
    ...
    const reviewerInput=frontierCandidates.length?{...reviewer,candidates:frontierCandidates,observations:reviewer.observations.filter(row=>frontierIds.has(row.candidate.candidate_id))}:reviewer;
    const reviewerResult=frontierCandidates.length?select(reviewerInput):allReviewerResult;
