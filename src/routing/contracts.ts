...
const treatmentSchema = z.object({
  ...
  provisional:provisionalEvidenceSchema.nullable(),
}).strict();
...
export const routingPackSchema = z.object({
  ...
  for(const route of pack.routes){
    ...
    if(!sorted(qualifiedReviewers.slice(['SELECT','RETAIN','PROMOTE'].includes(route.reviewer_decision.outcome)?1:0))||!sorted(qualifiedWorkers.slice(route.worker_decision.outcome==='RETAIN'||route.worker_decision.outcome==='PROMOTE'?1:0)))ctx.addIssue({code:'custom',path:['routes'],message:'treatment ladder is not cost ordered'});
  }
