...
export function renderClaude(input: RenderInput): RenderedAdapter {
  ...
  if (!["low", "medium", "high", "xhigh", "max"].includes(binding.candidate.effort)) throw new Error("UNSUPPORTED_CLAUDE_EFFORT");
  ...
  return finishRender(input, "anthropic", binding, path, {
    [path]: `---\n${stringify({ name, description, model: binding.candidate.model_id, effort: binding.candidate.effort, tools })}---\n${nativeInstructions(binding)}\n`,
