...
export function renderCodex(input: RenderInput): RenderedAdapter {
  ...
  return finishRender(input, "openai", binding, path, {
    [path]: TOML.stringify({ name, description, model: binding.candidate.model_id, ...(binding.candidate.effort === "not_applicable" ? {} : { model_reasoning_effort: binding.candidate.effort }), sandbox_mode: binding.role_id === "reviewer" ? "read-only" : "workspace-write", developer_instructions: nativeInstructions(binding) }),
