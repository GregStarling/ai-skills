...
import {compileRoutingPack,parseRoutingPack,resolveRouting,provisionalRouteInputSchema,validateRoutingPackPublication} from '../../routing/index.js';
...
export const commands=[
 jsonCommand('compile-routing-pack','Compile current governed worker and frontier evidence into a portable production pack.',async(input,cwd)=>{
  const request=z.object({policyFile:z.string().optional(),output:z.string().optional(),strata:z.array(z.object({publicTaskClass:publicClass,workerSelection:z.unknown(),reviewerSelection:z.unknown()}).strict()),provisional:z.array(provisionalRouteInputSchema).optional()}).strict().parse(input);
  const pack=validateRoutingPackPublication(compileRoutingPack({mode:'production',policy:loadPolicy(resolve(cwd,request.policyFile??'policy/constitution.json')),strata:request.strata,...(request.provisional?{provisional:request.provisional}:{})}));
 ...
 jsonCommand('validate-routing-pack','Validate routing pack integrity and publication timestamps against the current clock.',input=>{const pack=validateRoutingPackPublication(input);return {status:'VALID',mode:pack.mode,pack};}),
