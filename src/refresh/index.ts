...
export async function proposeRefresh(value:unknown){
 ...
 if(input.incumbentCandidateId!==undefined&&result.decision.rule_ids.includes('incumbent_not_qualified')){const {incumbentCandidateId:_invalid,...repair}=input;input=repair;result=select(input);incumbentStatus='INVALID';}
 ...
 const economics=result.qualifications.map(q=>({candidate_id:q.candidate_id,status:q.status,accepted:q.metrics.accepted,tasks:q.metrics.tasks,cost_per_accepted_task_usd:q.metrics.cost_per_accepted_task_usd,total_cost_usd:q.metrics.total_cost_usd}));
 ...
 const summary=`${diff.role_id}/${diff.task_class_id}: ${incumbentId??'unassigned'} → ${diff.proposed_candidate_id??'none'}; decision ${diff.decision_outcome}. Trigger: ${request.trigger}. Rules: ${diff.rule_ids.join(', ')}. Cost per accepted task: ${economics.map(q=>`${q.candidate_id}=${q.cost_per_accepted_task_usd===null?'unknown':q.cost_per_accepted_task_usd.toFixed(6)} USD (${q.status})`).join('; ')}. Evidence: ${diff.evidence_refs.length} raw observations.`;
