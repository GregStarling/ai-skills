...
describe('routing pack CLI boundaries',()=>{
 it('compiles an honest production gap pack and refuses to overwrite a staged artifact',async()=>{
  ...
  expect(result.exitCode).toBe(0);expect(result.body.status).toBe('COMPILED');expect(result.body.pack.mode).toBe('production');expect(result.body.pack.routes).toEqual([]);expect(result.body.pack.missing_routes).toHaveLength(8);
