...
The latest user clarification makes `skills/delegate/SKILL.md` the primary product: a standalone skill for any task, prioritizing high-quality output and then minimizing total effort through completion. Foreman and Model Governor are not prerequisites. Direct execution is the default (decided 2026-09-11; two ordinary-workflow pairs favored direct on API-equivalent estimates, while older pairs favored direct on elapsed time, not comparable usage; see docs/delegate-ordinary-results-2026-09-11.md). Automatically consider Delegate for substantive implementation, investigation, research, analysis, planning and review, not only coding. Exclude casual conversation, simple factual answers, creative drafting, voice-sensitive editing and worker/reviewer packets; explicit creative delegation remains opt-in, while bounded factual research and continuity checks may be considered separately. Activation is a routing check, not an automatic worker launch. Delegate only for large cross-package or source-based investigations, independent parallel workstreams or explicit user requests; treat the first two as untested until matched evidence shows savings, and do not reintroduce investigate-first as the default. Spend stronger reasoning on ambiguity and consequential decisions, preserve relevant tests, visual checks and required independent review, include coordination, failed attempts and repairs in efficiency judgments, and stop when the requested quality is achieved. Direct execution is ordinary host work, not a qualified pack route. Delegated work still requires eligible workers and frontier verification. The governor serves as maintainer-side routing-pack compiler. Native host tools intersect that portable pack with actual availability. Keep qualified routes evidence-backed and gaps explicit. See docs/routing-pack-plan.md for the earlier mandatory-delegation design, superseded by this clarification. Existing governor rules still apply when that engine is explicitly invoked or project policy requires it. Do not globally install skills or alter provider settings without authorization. The older engine release and its checks are not evidence of end-to-end standalone skill acceptance. See docs/standalone-delegate-plan.md.

## Local consumer feedback

Delegate now records direct and delegated outcomes through a dependency-free optional local helper. Personal history and reminder settings remain outside installations and repositories. Local preferences cannot qualify models, replace qualified incumbents, expand source scope, or weaken review. Session reminders are advisory at safe milestones, never automatic restarts or claims of measured savings. Live research requires separately demonstrated host/tool scope; supplied-source pilots are not web-research evidence. See docs/delegate-local-learning-plan.md.
