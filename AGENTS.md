...
The current user requirement is a standalone Delegate that preserves requested quality while minimizing subscription allowance across MVPs and large repositories. Ordinary host delegation is bounded by the assignment, permissions, actual model/effort controls, risk and verification—not a repository-language/size whitelist. `dispatch` uses declared economy/standard host slots; it makes no qualification or measured-savings claim. Direct execution is the default (decided 2026-09-11 after six matched comparisons all favored direct; see docs/delegate-ordinary-results-2026-09-11.md). Delegate only for large multi-package investigations, independent parallel workstreams or explicit user requests; treat the first two as untested until matched evidence shows savings, and do not reintroduce investigate-first as the default. Locate ownership, trace affected dependencies, assign coherent changes, then verify consumers/contracts and integrated behavior.
