...
The current user requirement is a standalone Delegate that preserves requested quality while minimizing subscription allowance across MVPs and large repositories. Ordinary host delegation is bounded by the assignment, permissions, actual model/effort controls, risk and verification—not a repository-language/size whitelist. `dispatch` uses declared economy/standard host slots; it makes no qualification or measured-savings claim. Direct execution is the default (decided 2026-09-11; two ordinary-workflow pairs favored direct on API-equivalent estimates, while older pairs favored direct on elapsed time, not comparable usage; see docs/delegate-ordinary-results-2026-09-11.md). Automatically consider Delegate for substantive implementation, investigation, research, analysis, planning and review, not only coding. Exclude casual conversation, simple factual answers, creative drafting, voice-sensitive editing and worker/reviewer packets; explicit creative delegation remains opt-in, while bounded factual research and continuity checks may be considered separately. Activation is a routing check, not an automatic worker launch. Delegate only for large cross-package or source-based investigations, independent parallel workstreams or explicit user requests; treat the first two as untested until matched evidence shows savings, and do not reintroduce investigate-first as the default. Locate ownership, trace affected dependencies, assign coherent changes, then verify consumers/contracts and integrated behavior.

This explicitly separates ordinary host work from evidence-required `route`/`lookup`. When user/project policy requires pack qualification, preserve literal scopes, expiry, identity, reviewer rules and gaps; never bypass that policy through ordinary dispatch. Do not rewrite old evidence or promote TypeScript/large-repository capability from JavaScript fixtures. Model Governor remains the strict maintainer-side evidence engine, not a prerequisite for ordinary skill use.

## Local consumer feedback

Ordinary delegated investigation and implementation use one `observe` call at completion, without start/capture/finish ceremony. Recent same-project/assignment/model outcomes surface failures and repairs; they are caller-reported feedback, not economic preferences or qualification. Full receipts remain for evidence-routed work and explicit tracked evaluations. Learned economic tie-breaks accept only complete comparable observed allowance or attributable cost; neither elapsed time nor raw tokens can select winners. Unknown usage stays unknown, and proxy estimates never establish subscription savings. Do not spawn models merely to measure usage.

Keep the installed consumer folder synchronized only when authorized. Source checkout stays inert. The current user authorized fixing these large-repository and feedback defects; historical evaluation budgets below do not authorize a new broad qualification campaign. Bounded local checks and usability forward-tests must report their actual scope without claiming general savings. See docs/delegate-routing-coverage.md.
